...
LastEditTime: 2024-12-06 22:54:48
...
if __name__ == '__main__':
	...
	parser.add_argument('-v', '--version', action='version', version='%(prog)s version: 2.2.5')
	...
	group1.add_argument('-md', '--min-descriptions', type=int, default=15, dest="", help="商品已有文案的最少数量(15) | 真实评论文案多余这个数脚本才会正常获取已有文案。")
	group1.add_argument('-mi', '--min-images', type=int, default=15, dest="", help="商品已有图片的最少数量(15) | 真实评论图片多余这个数脚本才会正常获取已有图片。")
	group1.add_argument('-mc', '--min-charcount', type=int, default=60, dest="", help="评论文案的最少字数(60) | 在已有评论中随机筛选文案的限制条件，JD:优质评价要求60字以上。")
	
	group2 = parser.add_argument_group(title="自动化设置")
	# 使用 store_true 动作：如果命令行传入 --auto-commit 参数，则 auto_commit 为 True，否则为 False
	group2.add_argument('-scpc', '--select-current-product-close', action='store_true', default=False, dest="", help="关闭仅查看当前商品 | 启用此设置，在获取已有评论文案与图片时将查看商品所有商品评论信息，关闭可能会导致评论准确性降低")
	group2.add_argument('-acc', '--auto-commit-close', action='store_true', default=False, dest="", help="关闭自动提交 | 启用此设置，在自动填充完评价页面后将不会自动点击提交按钮")

	group3 = parser.add_argument_group(title="AI设置", description="-g 与 -m 需同时设置;")
	group3.add_argument('-g', '--ai-group', type=str, default=None, dest="", help="AI模型的组别名称 | 使用AI模型生成评论文案")
	group3.add_argument('-m', '--ai-model', type=str, default=None, dest="", help="AI模型的名称 | 使用AI模型生成评论文案")
