...
class JDAppAutomaticEvaluate(object):
    ...
    def get_text(self, evaluation_texts: list[tuple[int, str]]) -> str | None:
        """
    ...
    @sync_retry(max_retries=3, retry_delay=2, backoff_factor=2, exceptions=(ValueError,))
    def get_image(self,  image_groups: list[list[tuple[int, str]]]) -> list | None:
        """
    ...
    def auto_fill(self, task: EvaluationTask) -> None:
        ...
        if not image_input_ele:
            ...
            if self.AUTO_COMMIT:
                ...
                LOG.warning(f"【AUTO_COMMIT】设置为{self.AUTO_COMMIT}，需手动提交评价！")
    ...
    def run(self):
        ...
        if not goto_evaluate_ele:
            ...
            self.current_task.input_text = self.get_text(inner_texts)
            LOG.debug(f"【input_text】{self.current_task.input_text}")
            self.current_task.input_image = self.get_image(inner_image_groups)
            LOG.debug(f"【input_image】{self.current_task.input_image}")
